<template>
  <div class="vplus-favorite">
    <div v-if="isEditing">
      <checklist
        :title="`全部${songList.length}首歌曲`"
        :options="sortName(songsToSelect) "
        :value.sync="selectedSongs"
        :required="false"
      ></checklist>
      <checklist
        title="已关注歌曲"
        :options="sortName(cachedFavoriteSongs)"
        :value.sync="selectedSongs"
        :required="false"
      ></checklist>
    ...
    <div v-else>
      ...
      <group
        :title="`共${favoriteSongs.length}首歌曲`"
        v-show="favoriteSongs.length > 0"
      >
        <cell
...
<script>
  import {Group, Cell, Checklist, Divider, XButton} from 'vux'
